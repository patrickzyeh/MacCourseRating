...
function Dashboard(props) {
    ...
    useEffect(() => {
        const fetchRatings = async () => {
            try {
                const response = await axios.get(`https://courseratingbackend.vercel.app/api/ratings/user/${props.user.email}`);
                                        ...
                                        return (
                                            <div key={rating.id}>
                                                <p className="dashboard-course-code">
                                                    <Link style={{ textDecoration: "none", color: "#7a003c" }} to={"/ratings/" + rating.course_code}>
                                                        {rating.course_code}
                                                    </Link>
                                                </p>
                                                <CourseRating
