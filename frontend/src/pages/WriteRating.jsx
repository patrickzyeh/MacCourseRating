...
function WriteRating(props) {
    ...
    useEffect(() => {
        const getPreviousRating = async () => {
            try {
                const response = await axios.get(`https://courseratingbackend.vercel.app/api/ratings/specific/${courseCode}/${props.user.email}`);
                    ...
                    if (!json.isProfanity) {
                        ...
                        try {
                            const response = await fetch(
                                `https://courseratingbackend.vercel.app/api/ratings/post/${courseCode}/${props.user.email}`,
                                {
                                    method: "POST",
            ...
            try {
                await fetch("https://vector.profanity.dev", {
                    ...
                    if (!json.isProfanity) {
                        try {
                            const response = await fetch(
                                `https://courseratingbackend.vercel.app/api/ratings/update/${courseCode}/${props.user.email}`,
                                {
                                    method: "PATCH",
                                    body: new URLSearchParams({
                                        easeRating: easeRating,
    ...
    return (
        <>
            ...
            <div className="course-container">
                <h2 className="write-title">{props.update ? "Update Your Rating for" : "Write a Rating for"}</h2>
            ...
            <div className="rating-page">
                ...
                <button
                    className={`post-button ${
                        easeRating && practicalityRating && enjoyabilityRating && overallRating && commentInput ? "button-enabled" : "button-disabled"
                    }`}
                    disabled={easeRating && practicalityRating && enjoyabilityRating && overallRating && commentInput ? false : true}
